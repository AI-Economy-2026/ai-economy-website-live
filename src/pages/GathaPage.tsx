...
import { ArrowLeft, ArrowRight, CheckCircle2, Layout, Mic, Newspaper, Search, ChevronRight, Bookmark, Filter, ChevronDown, ChevronUp } from 'lucide-react';
...
export function GathaPage({ product }: GathaPageProps) {
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Hero */}
      <section className="relative pt-14 min-h-[85vh] flex items-center overflow-hidden bg-[#0A0A0B] text-white">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 py-24 w-full">
          <motion.div
            ...
            className="max-w-3xl"
          >
            <p className="text-white/50 text-xs font-bold uppercase tracking-[0.2em] mb-6">Personal Knowledge Dashboard</p>
            <h1 className="text-6xl md:text-[90px] font-extrabold text-white tracking-tighter leading-[0.9] mb-8">
              Your digital life,<br />distilled.
            </h1>
      ...
      <section className="py-24 bg-[#F5F5F7]">
        <div className="max-w-7xl mx-auto px-6">
          ...
          >
            <h2 className="text-3xl md:text-5xl font-bold tracking-tight mb-4 text-black">Too much to read, too little time.</h2>
          </motion.div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-black">
            {[
              { icon: Bookmark, text: 'Bookmarks are where good articles go to die, lost in browser silos' },
              { icon: Filter, text: 'Newsletters clutter your inbox, mixing urgent work with deep reading' },
              { icon: Newspaper, text: 'The mental load of "I should read this" creates constant digital anxiety' }
            ].map((point, i) => {
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
            <motion.div
              ...
              viewport={{ once: true }}
              className="bg-[#0A0A0B] rounded-[40px] aspect-[4/3] p-8 relative overflow-hidden shadow-2xl"
            >
            ...
            >
              ...
              <p className="text-xl text-foreground/60 font-medium mb-12">
                Stop scrolling lists. Gatha creates a beautiful, searchable mosaic of your knowledge, parsed for readability and ready for action.
              </p>
              <div className="grid grid-cols-1 gap-6">
                {[
                  { icon: Search, title: 'Semantic Search', desc: 'Find ideas by what they mean, not just the keywords you remember.' },
                  { icon: Mic, title: 'Personal Podcast', desc: 'Turn your weekly reading list into a professional audio brief for your commute.' },
                  { icon: Newspaper, title: 'Synthesized Briefs', desc: 'AI-generated morning newsletters summarizing your saved content.' }
                ].map((item, i) => (
                  <div key={i} className="flex items-start space-x-4 p-6 rounded-3xl hover:bg-[#F5F5F7] transition-colors">
