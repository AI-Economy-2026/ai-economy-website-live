      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8">
        <div className="flex h-[56px] items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-0.5">
            {navLinks.map((link) =>
              link.internal ? (
                <Link key={link.label} href={link.href} className="px-3 py-1.5 text-sm text-foreground/60 hover:text-foreground transition-colors rounded-md hover:bg-[#F9FAFB]">
                  {link.label}
                </Link>
              ) : (
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {cats.map((cat, i) => (
            <Link
              key={cat.title}
              href={`/product/ai-toolkit/category/${cat.slug}`}
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-[450px_1fr] gap-16 items-center">
        ...
        <div className="flex flex-col animate-fade-in-up delay-200">
          ...
          <div className="space-y-6 text-xl md:text-2xl text-foreground/60 leading-relaxed font-medium mb-12">
            <p>Sarah tests every tool before recommending it. No sponsored rankings, no pay-to-play lists &mdash; just honest reviews from someone who&apos;s been in the AI space since day one.</p>
