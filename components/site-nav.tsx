      ...
      <div className="fixed inset-x-0 top-8 z-40 border-b border-black/5 bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex h-16 w-full max-w-[1200px] items-center justify-between px-6 md:px-12">
          ...
          <nav className="hidden items-center gap-8 md:flex">
            {navLinks.map((link) => (
              <Link
                ...
                className={`text-[13px] font-medium transition hover:text-black ${
                  pathname.startsWith(link.href) ? 'text-black' : 'text-black/70'
                }`}
              >
                {link.label}
              </Link>
            ...
            {isHome ? (
              <a href="#cta" className="text-[13px] font-medium text-black/70 transition hover:text-black">
                Contact
              </a>
            ) : (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center justify-center rounded-full bg-brandBlue px-5 py-2 text-[13px] font-semibold text-white transition hover:bg-[#003A8C]"
              >
                Contact
              </a>
            )}
